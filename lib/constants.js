const URLS = type => {
  ...
  return {
    ...
    USERNAME: (platform, query) =>
      `${V2}platformType=${platform}&idOnPlatform=${query.join(',')}`,
    ...
    PLAYTIME: (platform, query) =>
      `${base(platform)}playerstats2/statistics?statistics=casualpvp_timeplayed,rankedpvp_timeplayed,generalpvp_timeplayed&populations=${query.join(',')}`,
    ...
    RANK: (platform, query, season, region) =>
      `${base(platform)}r6karma/players?board_id=pvp_ranked&season_id=${season}&profile_ids=${query.join(',')}&region_id=${region}`,
  }[type];
};
...
const OPERATORS = [
  ...
  },
  {
    ...
    fullIndex: '',
    gadget: [
      { id: type => `operator${type}_xxx`, name: 'Total Distance Reeled' }
    ]
  },
  {
    name: 'goyo',
    ...
    fullIndex: '',
    gadget: [
      { id: type => `operator${type}_xxx`, name: 'Volcán Detonated by Your Team' }
    ]
  }
];
...
const STATS = {
  ...
  operatorGadgets: type => OPERATORS.slice(5).map(op => op.gadget.map(g => g.id(type))).flat(),
  thunt: () => [
...
// replace all icons once ubi update their site
const RANKS = {
  23: { name: 'Champions', badge: '' },
  22: { name: 'Diamond', badge: 'h02BrKN' },

  21: { name: 'Platinum 1', badge: 'p8J2gyx' },
  20: { name: 'Platinum 2', badge: '0nSeDwK' },
  19: { name: 'Platinum 3', badge: '27k46er' },

  18: { name: 'Gold 1', badge: 'YSMXwwj' },
  17: { name: 'Gold 2', badge: 'XmoIXvX' },
  16: { name: 'Gold 3', badge: 'ua1gXAC' },

  15: { name: 'Silver 1', badge: '5oHNIK6' },
  14: { name: 'Silver 2', badge: 'fCzjRiu' },
  13: { name: 'Silver 3', badge: 'BtHkA3t' },
  12: { name: 'Silver 4', badge: '9YKfOUm' },
  11: { name: 'Silver 5', badge: '' },

  10: { name: 'Bronze 1', badge: 'UL72TCz' },
  9: { name: 'Bronze 2', badge: 'ScN2Nq7' },
  8: { name: 'Bronze 3', badge: 'y9S2Dxp' },
  7: { name: 'Bronze 4', badge: 's7WbWgP' },
  6: { name: 'Bronze 5', badge: '' },

  5: { name: 'Copper 1', badge: 'N0Ni07K' },
  4: { name: 'Copper 2', badge: 'piH6pb4' },
  3: { name: 'Copper 3', badge: 'ngBCfAK' },
  2: { name: 'Copper 4', badge: 'yDpq16S' },
  1: { name: 'Copper 5', badge: '' },

  0: { name: 'Unranked', badge: 'bvnVUEm' }
};

// remove later Xd
Object.values(RANKS).map(rank => { if (!rank.badge) rank.badge = 'fbuVNmD' });


// oldBadge: old icon, don't change | badge: new icons since phantom sight, need update every icon below plat
const OLD_RANKS = {
...
Object.values(OLD_RANKS).map(rank => { if (!rank.badge) rank.badge = rank.oldBadge });
